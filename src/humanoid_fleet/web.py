          ...
          try {{
            ...
            if (grid && data.skills) {{
              grid.innerHTML = data.skills.map((skill) => {{
                const summary = (skill.summary || "").replace(/</g, "&lt;");
                const io = JSON.stringify(
                  {{ input: skill.input_schema, output: skill.output_schema }},
                  null,
                  2
                );
                return `
                  <article class="skill-card" data-category="${{(skill.category || '').replace(/"/g, "&quot;")}}">
                    <header>
                      <h4>${{(skill.name_zh || skill.name || "").replace(/</g, "&lt;")}}</h4>
                      <code>${{(skill.id || "").replace(/</g, "&lt;")}}</code>
                    </header>
                    <p>${{summary}}</p>
                    <dl class="skill-meta">
                      <dt>类别</dt><dd>${{(skill.category || "").replace(/</g, "&lt;")}}</dd>
                      <dt>归属机器人</dt><dd>${{(skill.owner_robot || "").replace(/</g, "&lt;")}}</dd>
                      <dt>状态</dt><dd><span class="skill-status status-${{(skill.status || "registered").replace(/</g, "&lt;")}}">${{(skill.status || "").replace(/</g, "&lt;")}}</span></dd>
                    </dl>
                    <details>
                      <summary>输入 / 输出 schema</summary>
                      <pre>${{io.replace(/</g, "&lt;")}}</pre>
                    </details>
                  </article>
                `;
              }}).join("");
              ...
              if (count) {{
                count.textContent = `${{data.skills.length}} 项`;
              }}
            }}
            skillsLoaded = true;
          }} catch (error) {{
...
# and the `skill_names` advertised in bootstrap.py. Read-only in the prototype —
# a future Skill Registry service will own editable CRUD.
...
def _serve_skills(environ, start_response):
    body = json.dumps({"skills": SKILL_CATALOG}, ensure_ascii=False, indent=2).encode("utf-8")
    start_response(
        "200 OK",
        [
            ("Content-Type", "application/json; charset=utf-8"),
...
def _render_sidebar_panels() -> str:
    ...
    <section class="sidebar-panel" data-panel="skills" hidden>
      <header class="sidebar-panel-head">
        <div>
          <h3>原子能力清单</h3>
          <p class="muted">与 <code>orchestrator.py</code> / <code>runtime.py</code> 当前引用的技能保持一致。原型阶段为只读浏览。</p>
        </div>
...
def application(environ, start_response):
    ...
    if environ.get("PATH_INFO", "") == "/api/skills":
